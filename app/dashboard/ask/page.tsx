...
function AskContent() {
    ...
    const { mutate, isLoading, error, reset } = useMutation<RagQueryResult, RagQueryInput>(
        (input) => ragService.queryRag(input),
    );

    const activeFilterCount =
        (projectId ? 1 : 0) + (userId ? 1 : 0) + (from ? 1 : 0) + (to ? 1 : 0);
        ...
        <div className="mx-auto flex max-w-3xl flex-col gap-6">
            <div>
                ...
                <p className="mt-1 text-sm text-gray-600">
                    Query your team&apos;s updates in natural language. Results are grounded in
                    cited sources.
                </p>
            </div>
            ...
            <form onSubmit={onSubmit} className="rounded-lg border border-gray-200 bg-white p-4">
                ...
                <div className="mt-3 flex items-center justify-between">
                    <p className="text-xs text-gray-500">
                        Press <kbd className="rounded border border-gray-300 bg-gray-50 px-1">Enter</kbd>{' '}
                        to ask,{' '}
                        <kbd className="rounded border border-gray-300 bg-gray-50 px-1">Shift+Enter</kbd>{' '}
                        for newline.
                    </p>

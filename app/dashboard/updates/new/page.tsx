...
const darkSelect =
  'w-full rounded-md border border-gray-700 bg-gray-800 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-orange-500';

const darkTextarea =
  'bg-gray-800 border-gray-700 text-white placeholder:text-gray-500 focus:ring-orange-500 focus:border-orange-500';
...
export default function NewUpdatePage() {
  ...
  const { mutate, isLoading, error } = useMutation<UpdateResponse, UpdateInput>((input) =>
    updateService.submitUpdate(input),
  );
  ...
  return (
    <div className="mx-auto mt-10 max-w-2xl">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-semibold text-white">Submit daily update</h1>
        <Link
      ...
      {showSuccess && (
        <div className="mb-4 rounded-lg border border-green-800/50 bg-green-950/60 p-3 text-sm text-green-400">
      ...
      <form
        onSubmit={onSubmit}
        className="space-y-5 rounded-lg border border-gray-800 bg-gray-900 p-6 shadow-sm"
      >
        <div>
          <label htmlFor="project" className="mb-1.5 block text-sm font-medium text-gray-300">
            Project
          </label>
          <select
            id="project"
        ...
        <div>
          <div className="mb-1.5 flex items-baseline justify-between">
            <label htmlFor="completed" className="block text-sm font-medium text-gray-300">
              Completed today
            </label>
        ...
        <div>
          <div className="mb-1.5 flex items-baseline justify-between">
            <label htmlFor="nextSteps" className="block text-sm font-medium text-gray-300">
              Next steps
            </label>
        ...
        <div>
          <label htmlFor="blockers" className="mb-1.5 block text-sm font-medium text-gray-300">
            Blockers
          </label>
          <Textarea
            id="blockers"
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            <label htmlFor="hoursSpent" className="mb-1.5 block text-sm font-medium text-gray-300">
              Hours spent
            </label>
            <Input
              id="hoursSpent"
        ...
        <div>
          <label className="mb-1.5 block text-sm font-medium text-gray-300">Confidence</label>
          <ConfidenceToggle value={confidence} onChange={setConfidence} />
        </div>

        {alreadySubmittedToday && (
          <div className="rounded-lg border border-amber-800/50 bg-amber-950/60 p-3 text-sm text-amber-400">
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <Button type="submit" variant="orange" disabled={!isValid || isLoading}>
...
function CharCount({ value, min }: { value: string; min: number }) {
  ...
  return (
    <span
      className={
        len === 0 ? 'text-xs text-gray-500' : ok ? 'text-xs text-green-400' : 'text-xs text-red-400'
      }
    >
      {len}/{min}
      {ok ? ' ✓' : ''}
